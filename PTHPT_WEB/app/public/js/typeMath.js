...
function findSuitableElement(IdElement) {
    ...
    for (let i = index; i >= 0; i--) {
        if (
            children[i].classList.contains("FormLaTex") ||
            children[i].classList.contains("FormInput")
        ) {
...
function CreateElementLatex(IdElement) {
    ...
    if (IdInOutput) {
        ...
        solutionFormOutput.insertBefore(
            imageOutImg,
            solutionFormOutput.children[0]
        );
    }
...
function CreateOptionAddButton(IdElement) {
    ...
    createNewFormLaTexOrFormInput.classList.add(
        "CreateNewFormLaTexOrFormInput"
    );
...
function InsertElementAfter(IndexElement, newElement, ParentElement) {
    ParentElement.insertBefore(
        newElement,
        ParentElement.children[IndexElement + 1]
    );
}

function submitContent() {
    const content = getContentOutput();
    var data = {
        key1: "value1",
        key2: "value2",
    };
